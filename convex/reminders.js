...
export const sendDailyPaymentReminders = mutation({
  ...
  handler: async (ctx, args) => {
    console.log("[CRON] sendDailyPaymentReminders job triggered");
    // Find all expenses with unpaid splits
    const expenses = await ctx.db.query("expenses").collect();
    console.log(`[CRON] Found ${expenses.length} expenses`);
    ...
    for (const expense of expenses) {
      for (const split of expense.splits) {
        if (!split.paid && split.userId !== expense.paidByUserId) {
          ...
          console.log(`[CRON] Unpaid split found: expense '${expense.description}', amount ${split.amount}, to ${owingUser?.email}`);
          if (owingUser && owingUser.email) {
            ...
            emailsAttempted++;
            const result = await ctx.runMutation(api.email.sendEmailMutation, {
              to: owingUser.email,
              subject,
              html,
              text: `Reminder: You owe ₹${split.amount} for '${expense.description}'. Please settle up with ${payer?.name || "the payer"}.`,
            });
            console.log(`[CRON] Email send result:`, result);
            if (result && result.success) emailsSent++;
          }
        }
      }
    }
    console.log(`[CRON] Emails attempted: ${emailsAttempted}, emails sent: ${emailsSent}`);
    return { success: true, emailsAttempted, emailsSent };
  }
});
